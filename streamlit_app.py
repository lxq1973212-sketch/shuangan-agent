...
严格真实学时打卡
"""
        self.extra_knowledge = ""   # 用户上传的新知识会追加在这里
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(f"✅ 已成功学习新知识！当前知识库已更新")
    ...
    with col2:
        if st.button("📥 下载当前完整知识库"):
            full_text = st.session_state.kb.get_full_knowledge()
            st.download_button("点击下载", full_text, file_name="内江双安驾校知识库.txt")
